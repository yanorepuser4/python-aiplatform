...
@pytest.fixture
def predict_client_stream_direct_predict_async_mock():
    response = (
        gca_prediction_service.StreamDirectPredictResponse(outputs=_TEST_OUTPUTS),
        gca_prediction_service.StreamDirectPredictResponse(outputs=_TEST_OUTPUTS),
    )
    with mock.patch.object(
        target=prediction_service_async_client.PredictionServiceAsyncClient,
        attribute="stream_direct_predict",
        return_value=response,
    ) as stream_direct_predict_mock:
        yield stream_direct_predict_mock
...
@pytest.fixture
def predict_client_stream_direct_raw_predict_async_mock():
    response = (
        gca_prediction_service.StreamDirectRawPredictResponse(output=_TEST_RAW_OUTPUTS),
        gca_prediction_service.StreamDirectRawPredictResponse(output=_TEST_RAW_OUTPUTS),
    )
    with mock.patch.object(
        target=prediction_service_async_client.PredictionServiceAsyncClient,
        attribute="stream_direct_raw_predict",
        return_value=response,
    ) as stream_direct_raw_predict_mock:
        yield stream_direct_raw_predict_mock
...
class TestEndpoint:
    ...
    def test_stream_direct_predict(self, predict_client_stream_direct_predict_mock):
        ...
        assert true_prediction == test_prediction
        predict_client_stream_direct_predict_mock.assert_called_once_with(
            endpoint=_TEST_ENDPOINT_NAME,
            inputs=_TEST_INPUTS,
            parameters=None,
            timeout=None,
    ...
    async def test_stream_direct_predict_async(
        self, predict_client_stream_direct_predict_async_mock
    ):
        test_endpoint = models.Endpoint(_TEST_ID)
        test_prediction_iterator = await test_endpoint.stream_direct_predict_async(
            inputs=_TEST_INPUTS, parameters=None
        )
        test_prediction = list(test_prediction_iterator)
        ...
        assert true_prediction == test_prediction
        predict_client_stream_direct_predict_async_mock.assert_called_once_with(
            endpoint=_TEST_ENDPOINT_NAME,
            inputs=_TEST_INPUTS,
            parameters=None,
            timeout=None,
    ...
    def test_stream_direct_predict_with_timeout(
        ...
        test_endpoint = models.Endpoint(_TEST_ID)
        test_prediction_iterator = test_endpoint.stream_direct_predict(
            inputs=_TEST_INPUTS, parameters=None, timeout=10.0
        )
        test_prediction = list(test_prediction_iterator)
    ...
    def test_stream_direct_raw_predict(
        ...
        assert true_prediction == test_prediction
        predict_client_stream_direct_raw_predict_mock.assert_called_once_with(
            endpoint=_TEST_ENDPOINT_NAME,
            method_name=_TEST_METHOD_NAME,
            input=_TEST_RAW_INPUTS,
            timeout=None,
    ...
    async def test_stream_direct_raw_predict_async(
        self, predict_client_stream_direct_raw_predict_async_mock
    ):
        test_endpoint = models.Endpoint(_TEST_ID)
        test_prediction_iterator = await test_endpoint.stream_direct_raw_predict_async(
            method_name=_TEST_METHOD_NAME, request=_TEST_RAW_INPUTS
        )
        test_prediction = list(test_prediction_iterator)
