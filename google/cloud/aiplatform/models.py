...
class Endpoint(base.VertexAiResourceNounWithFutureManager, base.PreviewMixin):
    ...
    async def stream_raw_predict_async(
        self, body: bytes, headers: Dict[str, str]
    ) -> Iterator[requests.models.Response]:
        """Makes a streaming prediction request using arbitrary headers.
        ...
        Example usage:
            my_endpoint = aiplatform.Endpoint(ENDPOINT_ID)
            async for stream_response in my_endpoint.stream_raw_predict_async(
                body = b'{"instances":[{"feat_1":val_1, "feat_2":val_2}]}'
                headers = {'Content-Type':'application/json'}
            ):
                status_code = response.status_code
    ...
    async def stream_direct_predict_async(
        self,
        inputs: List,
        parameters: Optional[Dict] = None,
        timeout: Optional[float] = None,
        ...
        """
        self.wait()
        async for resp in self._prediction_async_client.stream_direct_predict(
            endpoint=self._gca_resource.name,
            inputs=inputs,
            parameters=parameters,
            timeout=timeout,
    ...
    async def stream_direct_raw_predict_async(
        self,
        method_name: str,
        request: bytes,
        timeout: Optional[float] = None,
        ...
        Example usage:
            my_endpoint = aiplatform.Endpoint(ENDPOINT_ID)
            async for stream_response in my_endpoint.stream_direct_raw_predict(
                request=b'...'
            ):
                yield stream_response
        ...
        async for resp in self._prediction_async_client.stream_direct_raw_predict(
            endpoint=self._gca_resource.name,
            method_name=method_name,
            input=request,
            timeout=timeout,
